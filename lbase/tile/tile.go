...
func (this *NavigationMesh) Load(fileName string) bool {
	...
	bitStream := lbase.NewBitStream(buf, len(buf))
...
func (this *NavigationMesh) RandomPosition() (bool, lmath.Point3F) {
	x, y, time := 0, 0, 0
	for x, y = lbase.RandI(0, this.GetSizeX()-1), lbase.RandI(0, this.GetSizeY()-1); this.GetGridFlag(x, y) == 0; x, y = lbase.RandI(0, this.GetSizeX()-1), lbase.RandI(0, this.GetSizeY()-1) {
		time++
		if time > 100 {
			return false, lmath.Point3F{}
		}
