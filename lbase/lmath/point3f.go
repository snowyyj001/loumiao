...
func (this *Point3F) Interpolate(from *Point3F, to *Point3F, factor float32) {
	lbase.Assert(factor >= 0.0 && factor <= 1.0, "Out of bound interpolation factor")
	inverse := 1.0 - factor
...
func (this *Point3F) Add1(p *Point3F) {
	this.X += p.X
	this.Y += p.Y
	this.Z += p.Z
}
...
func (this *Point3F) Sub1(p *Point3F) {
	this.X -= p.X
	this.Y -= p.Y
	this.Z -= p.Z
}
...
func (this *Point3F) MulF1(f float32) {
	this.X *= f
	this.Y *= f
	this.Z *= f
}
...
func (this *Point3F) Mul1(p *Point3F) {
	this.X *= p.X
	this.Y *= p.Y
	this.Z *= p.Z
}
...
func (this *Point3F) Div1(f float32) {
	this.X /= f
	this.Y /= f
	this.Z /= f
}
...
func DistanceP3f(p1 *Point3F, p2 *Point3F) float32 {
