...
import (
	...
	"github.com/snowyyj001/loumiao/lbase/containers"
)
...
func TestIntComparator(t *testing.T) {
	...
	for _, test := range tests {
		actual := containers.IntComparator(test[0], test[1])
		expected := test[2]
		if actual != expected {
...
func TestStringComparator(t *testing.T) {
	...
	for _, test := range tests {
		actual := containers.StringComparator(test[0], test[1])
		expected := test[2]
		if actual != expected {
			t.Errorf("Got %v expected %v", actual, expected)
		}
...
func TestTimeComparator(t *testing.T) {
	...
	for _, test := range tests {
		actual := containers.TimeComparator(test[0], test[1])
		expected := test[2]
		if actual != expected {
			t.Errorf("Got %v expected %v", actual, expected)
		}
