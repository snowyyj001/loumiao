...
import (
	...
	"github.com/snowyyj001/loumiao/lagent"
...
func (self *GateServer) DoInit() bool {
	...
	if self.ServerType == network.CLIENT_CONNECT { //对外(login,gate)
		if lconfig.NET_WEBSOCKET {
			...
			self.pService.(*network.WebSocket).SetMaxClients(lconfig.NET_MAX_CONNS)
		} else {
			self.pService = new(network.ServerSocket)
			self.pService.(*network.ServerSocket).SetMaxClients(lconfig.NET_MAX_CONNS)
		}
		self.pService.Init(lconfig.NET_LISTEN_SADDR)
...
func (self *GateServer) DoStart() {
	...
	lnats.SubscribeAsync(ldefine.TOPIC_SERVER_LOG, llog.Tp_SetLevel)

	if lconfig.NET_NODE_TYPE != lconfig.ServerType_WEB_LOGIN {
		if self.pService != nil {
			lutil.Assert(self.pService.Start(), fmt.Sprintf("GateServer listen failed: saddr=%s", self.pService.GetSAddr()))
		}
	...
	llog.Infof("GateServer DoStart success: name=%s,saddr=%s,uid=%d", self.Name, lconfig.NET_GATE_SADDR, lconfig.SERVER_NODE_UID)
}
...
func (self *GateServer) DoOpen() {
	...
	need := lconfig.NET_NODE_TYPE == lconfig.ServerType_Account           //挑选网关和world给client需要
	need = lconfig.NET_NODE_TYPE == lconfig.ServerType_Gate               //挑选world给client需要
	need = need || lconfig.NET_NODE_TYPE == lconfig.ServerType_World      //挑选zone给client需要，其他主逻辑也可能需要
	need = need || lconfig.NET_NODE_TYPE == lconfig.ServerType_LOGINQUEUE //统计所有world在线人数需要
	need = need || lconfig.NET_NODE_TYPE == lconfig.ServerType_WEB_LOGIN  //挑选网关给client需要
	if need {                                                             //只让必要的节点参与状态监视，减少消息量
		//watch status
		var key string
		if lconfig.NET_NODE_ID == 0 { //跨服
			key = ldefine.ETCD_NODESTATUS
		} else { //只关心本服的节点信息
			key = fmt.Sprintf("%s%d", ldefine.ETCD_NODESTATUS, lconfig.NET_NODE_ID)
		}
	...
	var key string
	if lconfig.NET_NODE_ID == 0 { //跨服
		key = ldefine.ETCD_NODEINFO
	} else { //只关心本服的节点信息
		key = fmt.Sprintf("%s%d", ldefine.ETCD_NODEINFO, lconfig.NET_NODE_ID)
	}
	...
	nodemgr.ServerEnabled = true
	llog.Infof("GateServer DoOpen success: name=%s, saddr=%s, uid=%d", self.Name, lconfig.NET_GATE_SADDR, lconfig.SERVER_NODE_UID)
	//	}, false)
...
func (self *GateServer) serverStatusUpdate(key, val string, dis bool) {
	...
	if node.Uid > 0 && node.SocketActive == false && node.Type == lconfig.ServerType_RPCGate {
...
func (self *GateServer) newServerDiscover(key, val string, dis bool) {
	...
	if node.Type == lconfig.ServerType_WEB_LOGIN ||
		node.Type == lconfig.ServerType_WEB_GM ||
		node.Type == lconfig.ServerType_WebKeyPoint { //filter 他们不参与rpc调用，不参与集群组网，其实参与了也没啥影响
		return
	}
	...
	if node.Type == lconfig.ServerType_LOGINQUEUE {
		self.QueueServerUid = node.Uid //这里记录一下，方便转发client的排队网络消息
	}

	if node.Type == lconfig.ServerType_RPCGate { //发现了一个rpc server，咱作为客户端去连上它，参与rpc的狂欢
	...
	} else if lconfig.NET_NODE_TYPE == lconfig.ServerType_Gate { //网关和其他server建立一条专线，用来直接转发client的消息
		gateConn := node.Type == lconfig.ServerType_World                 //世界服
		gateConn = gateConn || node.Type == lconfig.ServerType_Zone       //战斗服
		gateConn = gateConn || node.Type == lconfig.ServerType_LOGINQUEUE //排队服
...
func (self *GateServer) enableClient(client *network.ClientSocket) bool {
	if client.Start() {
		llog.Infof("GateServer connected %s success", client.GetSAddr())
		return true
	} else {
		llog.Warningf("GateServer connect failed %s", client.GetSAddr())
		return false
	}
	return true
}
...
func (self *GateServer) onSocketDisconnected(socketId int) {
	if self.ServerType == network.SERVER_CONNECT { //the server lost connect with gate
		...
		if value, ok := self.sidAgents.LoadAndDelete(socketId); ok {
			user := value.(*lagent.LouMiaoAgent)
...
func (self *GateServer) GetClientAgent(socketId int) *lagent.LouMiaoAgent {
	if v, ok := self.sidAgents.Load(socketId); ok {
		return v.(*lagent.LouMiaoAgent)
	}
	return nil
}
...
func (self *GateServer) BindWorld(socketId, userId, worldUid int, tokenId string) {
	user := &lagent.LouMiaoAgent{
		UserId:     int64(userId),
		LobbyUid:   worldUid,
		ZoneUid:    0,
		MatchUid:   0,
		GateUid:    lconfig.SERVER_NODE_UID,
		SocketId:   socketId,
		WorldToken: tokenId,
	}
...
func (self *GateServer) BindZone(userId, zoneUid int) {
	if sid, ok := self.uidAgents.Load(userId); ok {
		if user, ok := self.sidAgents.Load(sid); ok {
			user.(*lagent.LouMiaoAgent).ZoneUid = zoneUid
		}
	}
}
